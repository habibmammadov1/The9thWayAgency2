...
export default function TeamMemberCard({ member, variants }: TeamMemberCardProps) {
  ...
  const hasSocials = !!(member.linkedinUrl || member.instagramUrl);
  const linkedin = member.linkedinUrl || (hasSocials ? undefined : "#");
  const instagram = member.instagramUrl || (hasSocials ? undefined : "#");
  ...
  const content = (
    <div className="group cursor-pointer">
          ...
          <div className="absolute inset-0 bg-[#0B0B0C]/60 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col items-center justify-center gap-4">
            {linkedin && (
              <div className="translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500 delay-100">
                <a href={linkedin} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white text-[#0B0B0C] flex items-center justify-center hover:bg-[#E4E2DF] transition-colors">
            ...
            {instagram && (
              <div className="translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500 delay-150">
                <a href={instagram} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white text-[#0B0B0C] flex items-center justify-center hover:bg-[#E4E2DF] transition-colors">
      ...
      <div className="text-center md:text-left">
        ...
        <p className="text-[#8A8A87] text-sm">
          {member.role || t(`roles.role${member.id}`)}
        </p>
      </div>
    </div>
  );
